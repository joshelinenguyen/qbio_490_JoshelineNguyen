...
selected_hp <- mtcars[c("Duster 360", "Lincoln Continental", "Maserati Bora"), "hp"]
selected_hp
```
...
# write code here
colnames(mtcars)
df_copy <- mtcars[, -which(colnames(mtcars) == "am")]
```
...
```{r}
clin_query <- GDCquery(project = "TCGA-BRCA", data.category = "Clinical", file.type = "xml") #Fill in correct command
  
clinic <- GDCprepare_clinic(clin_query, clinical.info = "patient") #Fill in correct command
...
```{r}
write.csv(clinic, "/Users/joshelinenguyen/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/joshelinenguyen/analysis_data/brca_clinical_data.csv")
```
...
$gender is a chr data type.

# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)?
The 5th patient is 18371 days old (50.3 years).
...
```{r Exercise 4.1}
# create plot here
plot(clinic$days_to_birth, clinic$her2_neu_chromosone_17_signal_ratio_value, main = "Her2 Neu Chromosone 17 Signal Ratio Value vs. Age", xlab = "Days to Birth", ylab = "Her2 Neu Chromosone 17 Signal Ratio Value", col = 'blue')
```
...
```{r Exercise 4.2}
# create plot here
boxplot(clinic$days_to_last_followup, main = "Days to Last Follow Up", ylab = "Number of Days", col = 'purple')
```
...
```{r}
under30_over60_mask <- ifelse(((clinic$age_at_initial_pathologic_diagnosis > 35) & (clinic$age_at_initial_pathologic_diagnosis > 60)), T, F)
```
...
#The age KM plot had a p value of <0.0001 which means that the results were significant suggesting that patients diagnosed # when they were young had a higher probability of survival, followed by middle and lastly old.
