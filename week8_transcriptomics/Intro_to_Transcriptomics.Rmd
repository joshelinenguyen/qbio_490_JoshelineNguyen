...
rna_clinical$age_at_initial_pathologic_diagnosis <- as.numeric(rna_clinical$age_at_diagnosis)

rna_clinical$paper_BRCA_Subtype_PAM50 <- factor(rna_clinical$paper_BRCA_Subtype_PAM50)

NA_mask <- !is.na(rna_clinical$definition) & !is.na(rna_clinical$age_at_diagnosis)
  
rna_clinical_clean <- rna_clinical[NA_mask, ]
rna_counts_clean <- rna_counts[, NA_mask]
...
```{r}
colnames(rna_counts_clean) <- rna_clinical_clean$barcode
dds <- DESeqDataSetFromMatrix(countData = rna_counts_clean,
                              colData = rna_clinical_clean,
                              design= ~ age_at_initial_pathologic_diagnosis + definition)
...
top-right genes: upregulated genes, pretty significant
bottom-right genes: upregulated genes, significant
top-left genes: downregulated genes, pretty significant
bottom-left genes: downregulated genes, significant
top-middle genes: not significant
